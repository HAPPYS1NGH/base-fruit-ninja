...
export default function FruitNinjaGame() {
  ...
  const [followerScores, setFollowerScores] = useState<{
    [key: string]: number;
  }>({});
  ...
  const adjustCanvasSize = useCallback(() => {
    if (canvasRef.current) {
      ...
      canvas.height =
        window.innerHeight - canvas.getBoundingClientRect().top - 20; // Set height to fill available space
  ...
  useEffect(() => {
    ...
    gameLoopRef.current = (timestamp: number) => {
      ...
      const newTimeLeft = Math.max(
        0,
        Math.ceil(GAME_DURATION / 1000 - elapsedTime),
      );
      ...
      if (timestamp - lastSpawnTimeRef.current > spawnRate) {
        ...
        const fruitsToSpawn =
          Math.floor(Math.random() * MAX_FRUITS_PER_SPAWN) + 1;
        ...
        for (let i = 0; i < fruitsToSpawn; i++) {
          ...
          if (
            FRUITS[fruitTypeIndex].name !== "bomb" ||
            Math.random() < (fruitsToSpawn === 1 ? 0.1 : 0.05)
          ) {
            const newFruit = new Fruit(canvas.width, FRUITS[fruitTypeIndex]);
      ...
      fruitsRef.current = fruitsRef.current.filter((fruit) => {
        const isVisible = fruit.y < canvas.height + 100 || fruit.velocityY < 0;
        const isRecentlySliced =
          fruit.sliced && timestamp - fruit.sliceTime < 1000;
    ...
    try {
      ...
      if (isHighScore && context) {
        try {
          ...
          previousTopScorer = leaderboardData.find(
            (entry) => entry.fid !== context.user.fid && entry.score < score,
          );
      ...
      for (const victim of topVictims) {
        if (victim.fid) {
          try {
            fetch("/api/notify", {
              ...
              body: JSON.stringify({
                ...
                notification: {
                  ...
                  body: `You were sliced on Facebreaker by @${context?.user.username || "someone"}!`,
            ...
            console.error(
              "Error sending notification to victims :",
              victim.username,
              error,
            );
          }
      ...
      if (isHighScore && previousTopScorer && previousTopScorer.fid) {
        try {
          ...
          console.error(
            "Error sending notification to previous top scorer:",
            previousTopScorer.username,
            error,
          );
        }
      }
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center overflow-y-auto">
              ...
              <div className="w-full max-w-md flex flex-col items-center">
                <div className="backdrop-blur-xl bg-black/40 rounded-3xl p-6 w-full flex flex-col items-center">
                  <h3 className="text-2xl text-white font-bold mb-4 text-center font-gotens">
                    Top Slices
                  </h3>
                    ...
                    {Object.entries(followerScores)
                      ...
                      .map(([name, score], index) => {
                        ...
                        return (
                          <div
                            key={name}
                            className="bg-white rounded-xl shadow-md w-full flex items-center px-2 py-1 gap-3"
                          >
                            {/* Rank */}
                            ...
                            <div className="flex-grow min-w-0">
                              <h3 className="text-[15px] font-inter font-bold text-black truncate">
                                {name}
                              </h3>
                            </div>
                            {/* Score */}
                  ...
                  <div className="flex flex-col gap-3 w-full mt-2">
                    <button
                      className="bg-white text-tangerine-500 px-8 py-3 rounded-xl text-2xl md:text-xl shadow-lg hover:text-tangerine-600 transition-all font-gotens"
                      onClick={startGame}
                    >
                      Play again
                    </button>
