...
const RankBadge = ({ rank, size = 18 }: { rank: number; size?: number }) => {
  if (rank === 1) {
    return (
      <div className="bg-[#FFD700] p-1 rounded-full">
        <Trophy color="#fff" size={size} />
      </div>
    );
  } else if (rank === 2) {
    return (
      <div className="bg-[#C0C0C0] p-1 rounded-full">
        <Trophy color="#fff" size={size} />
      </div>
    );
  } else if (rank === 3) {
    return (
      <div className="bg-[#CD7F32] p-1 rounded-full">
        <Trophy color="#fff" size={size} />
      </div>
    );
  }
...
export default function LeaderboardPage() {
  ...
  return (
    ...
    >
      <div
        className="min-h-screen w-full max-w-xl mx-auto p-4 pt-0 flex flex-col"
      ...
      >
        ...
        {/* Content Section */}
        <div className="flex-grow" style={{ paddingBottom: "90px" }}>
          {loading ? (
        ...
        {/* Back button */}
        <div
          className="fixed bottom-0 left-0 w-full px-4  z-20 pb-4 pt-2"
          style={{
            backgroundImage: `url('/board.png')`,
            backgroundSize: "cover",
            backgroundPosition: "center",
        ...
        >
          <Link
            href="/"
            className="bg-tangerine-500 text-white px-6 py-3 rounded-xl text-3xl text-center hover:bg-tangerine-600 transition-all shadow-lg  block"
            style={{ width: "100%" }}
          >
            Back to Game
          </Link>
